...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <table class="w-full">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                        <select onchange="updateLinks(this, '{{ $report->report_id }}')" class="template-select text-xs border-gray-200 rounded-lg py-1 pl-2 pr-6">
                            <option value="default">Default</option>
                            <option value="modern1">Modern 1</option>
                            <option value="modern2">Modern 2</option>
                        </select>

                        <select onchange="updateLinks(this, '{{ $report->report_id }}')" class="header-select text-xs border-gray-200 rounded-lg py-1 pl-2 pr-6 ml-1">
            ...
            @empty
            <tr>
                <td colspan="5" class="px-6 py-8 text-center text-gray-500">No reports found</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
@push('scripts')
<script>
    function updateLinks(element, reportId) {
        // Find the row container to get both selects
        const container = element.closest('div');
        const template = container.querySelector('.template-select').value;
        const header = container.querySelector('.header-select').value;
        ...
        viewUrl.searchParams.set('header', header);
        viewUrl.searchParams.set('stream', 'true');
        viewBtn.href = viewUrl.toString();

        // Update Download Link
        let dlUrl = new URL(dlBtn.href);
        dlUrl.searchParams.set('template', template);
        dlUrl.searchParams.set('header', header);
