    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="font-semibold">Tests & Results</h3>
            </div>
            <table class="w-full">
        ...
        <div class="bg-primary-50 border border-primary-200 rounded-2xl p-6 text-center">
            <p class="font-semibold text-primary-700 mb-3">All Results Complete</p>

            <div class="flex items-center justify-center gap-4 mb-4">
                <select id="headerOption" class="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    <option value="yes">With Header (Default)</option>
                    <option value="no">Without Header (Pre-printed)</option>
                </select>
            </div>

            <a href="{{ route('reports.generate', $booking) }}" onclick="this.href = this.href.split('?')[0] + '?header=' + document.getElementById('headerOption').value" class="inline-block px-6 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 font-medium transition-colors">
                Generate Report
            </a>
        </div>
        @endif
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                @php
                $labName = auth()->user()->lab?->name ?? 'PathLAS';
                $receiptLink = route('bookings.receipt', $booking);

                $waMsg1 = "*{$labName}*\n\nDear Sir/Ma'am,\nThank you for visiting us!\n\n*Booking Confirmation*\n- Booking ID: {$booking->booking_id}\n- Patient: {$booking->patient->name}\n\nYou can download your receipt here:\n";
                $waMsg2 = "\n\nReport will be ready within 24-48 hours.\nThank you!";

                $fullWaMsg = $waMsg1 . $receiptLink . $waMsg2;
                $waReceiptUrl = "https://wa.me/?text=" . urlencode($fullWaMsg);
                @endphp
                <a href="{{ $waReceiptUrl }}" id="whatsappReceiptBtn" target="_blank"
                    data-msg-part1="{{ urlencode($waMsg1) }}"
                    data-base-link="{{ $receiptLink }}"
...
<script>
    function updateReceiptLink() {
        ...
        if (waBtn) {
            const msgPart1 = decodeURIComponent(waBtn.dataset.msgPart1);
            const baseLink = waBtn.dataset.baseLink;
            const msgPart2 = decodeURIComponent(waBtn.dataset.msgPart2);
